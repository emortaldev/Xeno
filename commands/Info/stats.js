...
module.exports.run = async (client, message) => {
    const heapUsed = Math.floor(process.memoryUsage().heapUsed / 1048576);
    const rssUsed = Math.floor(process.memoryUsage().rss / 1048576);

    message.channel.send({
        embed: {
            fields: [
                { name: "Ping", value: `🖥️ Ping \`${client.ws.ping}ms\`\n⏱️ Uptime \`${ms(Math.floor(client.uptime / 1000) * 1000)}\``, inline: true },
